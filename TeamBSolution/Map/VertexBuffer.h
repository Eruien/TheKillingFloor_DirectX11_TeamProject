...
public:
	...
	void UpdateVertexBuffer(const std::vector<T>& vertexData)
	{
		...
		HRESULT hr = LGlobal::g_pImmediateContext->Map(vertexBuffer.Get(), 0, D3D11_MAP_WRITE_DISCARD, 0, &subResource);
		...
		LGlobal::g_pImmediateContext->Unmap(vertexBuffer.Get(), 0);
	}
